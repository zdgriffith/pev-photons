...
if __name__ == "__main__":
    ...
    if args.extension:
        inFile = utils.prefix + 'all_sky/ext/p_value_skymap_ext_%s.npy' % args.extension
        outFile = 'all_sky_scan_ext_%s.png' % args.extension
    else:
        inFile = utils.prefix + 'all_sky/p_value_skymap.npy'
    ...
    scatter_args = {'cmap':utils.ps_map, 's':2, 'lw':0, 'zorder':0,
    ...
    ax.legend()
    plt.savefig(utils.fig_dir+'all_sky/'+outFile)
    if args.HESE:
        plt.savefig(utils.fig_dir+'paper/hese_events.pdf')
    elif not args.extension:
        plt.savefig(utils.fig_dir+'paper/all_sky_scan.pdf')
    plt.close()
