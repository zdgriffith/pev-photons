...
if __name__ == "__main__":
    ...
    p.add_argument("--seed", type=int, default=1,
                   help='rng seed')
    p.add_argument("--reco", help='reconstruction')
    args = p.parse_args()

    exp, mc, livetime, ps_llh = load_systematic_dataset('point_source', args.reco, args, reco_test=True)
    dec_list = np.radians(np.linspace(-84., -54., 100))
    ...
    for index in [2.0]:
        ...
        for j, dec in enumerate(dec_list):
            ...
            np.save(prefix+'systematics/{}_sens_index_{}.npy'.format(args.reco, index), sens)
            np.save(prefix+'systematics/{}_disc_index_{}.npy'.format(args.reco, index), disc)
