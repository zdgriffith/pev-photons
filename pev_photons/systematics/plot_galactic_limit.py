...
def plot_arrow(exp):
    for i, x_i in enumerate(exp['data'][0]):
        ax0.plot(np.full(2, x_i),
                np.full(2, exp['data'][1][i])*[1,0.5],
                color=exp['color'], zorder=2)
                
    ax0.scatter(exp['data'][0],exp['data'][1]*0.5,
               marker='v', color=exp['color'], s=40, zorder=2)
    ax0.scatter(exp['data'][0],exp['data'][1], label=exp['label'],
...
def plot_range(exp):
    ...
    ax0.plot(x, y, ls='--', color=exp['color'], zorder=1)
...
def plot_flux_model(model):
    ...
    ax0.fill_between(values['energy'], values['flux_low'], values['flux_high'],
...
if __name__ == "__main__":
    ...
    sybll_flux = np.load(prefix+'template/sybll_sens.npy')['flux']*2e6**2
    qs_flux = np.load(prefix+'template/qgs_sens.npy')['flux']*2e6**2
    exp['IC86 (SYBLL)'] = {'data':np.array([[2e3], [sybll_flux]]),
                           'color':colors[0], 'marker':'p',
                           'E_0':2e3, 'E_min':0.683e3, 'E_max': 2.73e3,
                           'convert': False, 'label':'IceCube 5 years'}

    exp['IC86 (QGS-JET)'] = {'data':np.array([[2e3], [value]]),
                             'color':colors[3], 'marker':'p',
                             'E_0':2e3, 'E_min':0.683e3, 'E_max': 2.73e3,
                             'convert': False, 'label':'IceCube 5 years'}
    ...
    ax1.scatter(2e6, exp['IC86 (SYBLL)']['data'][1][0]/exp['IC86 (QGS-JET)']['data'][1][0])
    ax1.set_xscale('log')
    ax1.set_xlabel(r'$E_\gamma$ [TeV]')
    ...
    if args.fermi_limit:
        ax0.set_ylabel(r'$E^2\Phi_{template}$ [GeV cm${}^{-2}$ s${}^{-1}$]')
        plt.savefig(fig_dir+'template/fermi_limit_sys.pdf')
    else:
        ax0.set_ylabel(r'$E^2J_\gamma$ [GeV cm$^{-2}$ s$^{-1}$ sr$^{-1}$]')
        plt.savefig(fig_dir+'template/per_str_limit_sys.pdf')
    plt.close()
