...
########################################################################
# Run the HESE cascade correlation analysis
########################################################################
...
def run_bg_trials(template_llh, args):
    """ Run background trials for the template analysis """
    trials = template_llh.do_trials(args.bg_trials)
    np.save(prefix+'HESE/trials/%s_job_%s.npy' % (args.name, args.job),
            trials['TS'])
...
def run_template_test(template_llh, args):
    ...
    np.save(prefix+'HESE/'+args.name+'_fit_result.npy', fit_arr)

if __name__ == "__main__":

    p = argparse.ArgumentParser(
            description='HESE cascade test',
            formatter_class=argparse.RawDescriptionHelpFormatter)
    p.add_argument('--name', type = str, default='cascades',
